...
public class PlayerMovementScript : MonoBehaviour {
    ...
    [Tooltip("Audio clip played while walking.")]
	...
	*/
    void WalkingSound()
    {
        // Ensure that the audio clips are assigned
        if (_walkSound == null)
        {
            Debug.Log("Missing walk sound.");
            return;  // Exit if walk sound is not assigned
        }
        ...
        if (RayCastGrounded())
        {
            ...
            {
                // If walking sound is not already playing, play it
                if (!_walkSound.isPlaying)
                {
                    _walkSound.clip = walkAudioClip; // Set the audio clip for walking
                    _walkSound.Play(); // Play walk sound
                }
            }
            else
            {
